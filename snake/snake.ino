/**
 * ------------------------------------------------------------------------------*
 * File: snake.ino   Implementação do jogo da cobrinha para Arduino/PQDB
 ...
 *
 *
 * Essa implementação supõe o uso do shield PQDB Mini. Porém, nem todos os
 * componentes da mesma são usados.
 * Para que ele funcione, é necessário ter 4 botões e o display Nokia 5110.
 * Para instruções das ligações do circuito, conferir o esquema da PQDB no
...
// Cada nó (gominho) da cobrinha contém uma lista de coordenadas x e y e o
// próximo movimento que o nó fará.
typedef struct {
  unsigned char x;
  unsigned char y;
  char nextMove;
...
unsigned char game[21][12];  // O "mapa" do jogo é uma matriz onde cada posição
                             // pode conter cobra (SNAKE), comida (FOOD) ou
                             // estar vazia (EMPTY).
char nextMove;  // Armazena o input do usuário, que será o próximo movimento da
                // cabeça da cobra.
...
// A função gameInit() inicializa todas as variáveis. Coloca a cobrinha em
// posição +- centralizada e limpa o mapa.
void gameInit() {
  food = false;
  foodX = 0;
  foodY = 0;
...
// Ela lê se/qual tecla o usuário está pressionando. Se for uma tecla
// direcional, ela será o próximo movimento da cabeça. Se for uma tecla
// inválida, será ignorada (a cobrinha continuará na direção anterior). O loop
// dessa função garante que o programa perderá algum tempo fazendo a leitura da
// tecla. Sem este loop, o programa passa tão pouco tempo nessa função que
// muitas vezes perde o acionamento de uma tecla.
void loop_Input() {
  for (unsigned char j = 0; j < 20; j++) {
    nextMove = kpReadKey();
    if (nextMove != 'U' && nextMove != 'D' && nextMove != 'L' &&
        nextMove != 'R')
      nextMove = 0;
  }
}
...
// Ela é responsável por atualizar todas as informações do jogo, na seguinte
// ordem:
// -> Limpa o mapa do jogo.
// -> Posiciona a comida no mapa.
// -> Atualiza os movimentos:
// 	  -> A cabeça recebe o input do usuário, caso tenha sido válido. Senão,
// segue na mesma direção.
//	  -> Os outros gomos recebem o movimento realizado pelo nó anterior a
// ele na rodada anterior a ele.
//    	 (Ex: se a cabeça vira à esquerda em uma rodada, o 2o gomo irá virar à
//    esquerda na rodada seguinte, o 3o depois de 2 rodadas...)
...
// -> Se a cabeça da cobra encontrou a comida, adiciona um novo gomo ao fim da
// cobra e sorteia uma nova posição para aparecer comida.
//    -> A posição do novo gomo é sempre atrás do último gomo.
//		 (Ex: se o último gomo está se movendo para a direita, o novo
// gomo aparecerá à sua esquerda.)
//    -> Se a cobra atingir o comprimento de 252 (todas as posições do mapa são
//    cobra), o jogador vence.
//	  -> A nova comida não pode aparecer sobre a cobra. Caso isso aconteça,
// uma nova posição é sorteada até que seja encontrada uma vazia.
void loop_Update() {
  // Limpa o mapa
  for (int i = 0; i < 21; i++) {
    for (int j = 0; j < 12; j++) {
  ...
  if (nextMove) snake[0].nextMove = nextMove;
  ...
  for (unsigned char i = 0; i < snakeLength; i++) {
    ...
    // Usa signed cast para detectar valores negativos
    if ((char)snake[i].x >= 21 || (char)snake[i].y >= 12)
      gameOver = true;
  ...
  // Se não há comida (e o usuário não atingiu o tamanho máximo), sorteie nova
  // comida
  if (!food && !win) {
    do {
      foodX = random(0, 21);
...
// -> Caso o usuário tenha perdido, mostra a mensagem de Game Over e reinicia o
// jogo.
// -> Caso o usuário tenha vencido, mostra a mensagem de vitória e reinicia o
// jogo.
...
//	  -> Os gomos da cobrinha são quadrados preenchidos, enquanto a comida é
// um quadrado só com as bordas.
// -> O atraso ao final da função ajuda a desacelerar um pouco o jogo. Pode ser
// ajustado para deixá-lo mais fácil ou mais difícil.
void loop_Draw() {
  if (gameOver) {
    clearScreen();
    LcdXY(15, 2);
...
// Nessa função é feita a inicialização do hardware, da seed dos sorteios e do
// jogo em si.
void setup() {
