...
void initializeDisplay(byte vop, byte tempCoef, byte bias) {
  ...
  LcdWriteCmd(0x21);  // LCD extended commands 0010 0001 --- 0010 PD V H
                      // PowerDown = 0, V = 0, H = 1
  // V = 0 para ativar escrita horizontal
  // H = 1 para ativar comandos adicionais

  LcdWriteCmd(vop);  // set LCD Vop (contrast) 1011 1111 Constrate setado para
                     // C0 = 5V Example: If user wants value of VLCD at 5 volt
  ...
  LcdWriteCmd(tempCoef);  // set temp coefficent 0000 0100 Coeficiente de
                          // temperatura para o 3
  // empirico
...
void setText(int x, int y, char* characters) {
  int shift = 0;
  while (*characters) {
    if (x > 0 && (x + shift < LCD_WIDTH - 6) && (y < LCD_HEIGHT - 8) && y > 0) {
