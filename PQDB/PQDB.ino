...
void loop() {
  ...
  for (key = 0; key < 4; key++) {  // Varredura
    if (kpRead() & (1 << key)) {   // Identifica qual tecla foi pressionada
      LcdWriteCharacter(key + 48);
      ssdDigit(3, key % 10);           // Unidade
      ssdDigit(2, key / 10 % 10);      // Dezena
      ssdDigit(1, (key / 100) % 10);   // Centena
      ssdDigit(0, (key / 1000) % 10);  // Milhar
