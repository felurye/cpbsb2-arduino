...
void timerStart(unsigned int time) { targetTime = millis() + time; }

// Aguarda até que a contagem termine
void timerWait(void) { while (targetTime > millis()); }

// Verifica se a contagem já terminou
// Retorna 1 se o tempo passou, 0 se ainda está contando
int timerFinished(void) { return targetTime <= millis() ? 1 : 0; }
