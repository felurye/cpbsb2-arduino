...
void turnOn(int led) {
    if (led & 1) { digitalWrite(LED_RED_PIN, HIGH); }
    if (led & 2) { digitalWrite(LED_GREEN_PIN, HIGH); }
    if (led & 4) { digitalWrite(LED_BLUE_PIN, HIGH); }
}

// Apaga os LEDs especificados pela mask
void turnOff(int led) {
    if (led & 1) { digitalWrite(LED_RED_PIN, LOW); }
    if (led & 2) { digitalWrite(LED_GREEN_PIN, LOW); }
    if (led & 4) { digitalWrite(LED_BLUE_PIN, LOW); }
}
